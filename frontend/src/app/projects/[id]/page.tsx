                ...
                <div key={score.dimension} className="bg-white rounded-2xl shadow p-6 flex flex-col">
                  <div className="flex items-center mb-2">
                    ...
                    {isEditing ? (
                      <input
                        ...
                        className="ml-auto w-16 px-2 py-1 rounded border border-gray-300 text-right text-gray-800 text-base focus:ring-2 focus:ring-purple-400 focus:border-purple-400"
                      />
                    ) : (
                  ...
                  {isEditing ? (
                    <textarea
                      ...
                      className="text-gray-800 text-base mb-2 p-3 rounded border border-gray-300 resize-none w-full focus:ring-2 focus:ring-purple-400 focus:border-purple-400"
                      rows={3}
                    />
                  ) : (
                  ...
                  {missing && (
                    <div className="text-xs text-yellow-500"><i className="fa-solid fa-circle-exclamation mr-1"></i> 缺失：{missing.description}</div>
                  )}
                </div>
              );
            })}
            ...
            {isEditing ? (
              <>
                <button 
                  ...
                  className="px-6 py-3 rounded-xl border border-gray-300 text-gray-600 font-semibold text-lg hover:bg-gray-50 transition"
                >
                  取消
                </button>
                <button 
                  ...
                  {historyList.length === 0 ? (
                    ...
                    historyList.map(item => (
                      <tr key={item.id} className="border-t hover:bg-gray-50">
                        ...
                        <td className="py-4 text-gray-700 text-base">{item.status === 'completed' ? '已完成' : item.status === 'processing' ? '待评审' : '补充信息'}</td>
                      </tr>
                    ))
                  )}
                </tbody>
